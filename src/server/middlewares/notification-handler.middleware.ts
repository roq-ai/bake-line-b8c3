...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order_created.create': { roles: ['business-owner', 'admin'], key: 'order_id', tenantPath: [], userPath: [] },
  'payment_received.create': { roles: ['business-owner', 'admin'], key: 'payment_id', tenantPath: [], userPath: [] },
  'stock_updated.update': { roles: ['business-owner', 'admin'], key: 'item_id', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['admin'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['business-owner', 'admin', 'customer'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.bakery.findFirst({
